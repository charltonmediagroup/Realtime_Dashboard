...
export default function EditorialLeaderboard({
  ...
  const fontSize = `clamp(0.8rem, min(calc(0.6vw + ${7 / effectiveCount}vw), ${dLineVh * 0.8}dvh), ${nameMax})`;
  const headerSize = `clamp(0.7rem, min(calc(0.4vw + ${4.5 / effectiveCount}vw), ${rowHeightVh * 0.2}dvh), 1.6rem)`;
  const storySize = `clamp(0.68rem, min(calc(0.45vw + ${5.5 / effectiveCount}vw), ${dLineVh * 0.7}dvh), ${storyMax})`;
  ...
  const chipSize = `clamp(0.62rem, min(calc(0.35vw + ${3.5 / effectiveCount}vw), ${dLineVh * 0.55}dvh), ${chipMax})`;
  ...
  return (
    ...
    >
      ...
      <div className="hidden md:flex landscape-show flex-col flex-1 min-h-0">
        <table
          className="lb-table w-full border-collapse table-fixed h-full"
          style={{ fontSize }}
        >
          <thead>
            <tr
      ...
      <div className="flex md:hidden landscape-hide flex-col flex-1 min-h-0">
        <table
          ...
          style={{ fontSize: `clamp(0.7rem, min(calc(0.8vw + ${6 / effectiveCount}vw), ${mLineVh * 0.72}vh), 2rem)` }}
        >
          <thead>
            <tr
              className="text-left font-semibold uppercase"
